...
def msssim(img1, img2, window_size=11, size_average=True, val_range=None, normalize=False, **kwargs):
    device = img1.device
...
class SSIM(torch.nn.Module):
    ...
    def forward(self, img1, img2):
        ...
        if self.channel == channel and self.window.dtype == img1.dtype and \
                self.window.device == img1.device:
            window = self.window
        else:
